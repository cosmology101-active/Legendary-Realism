...
execute as @a[tag=crafting_player] store result score @s temp_damage if data entity @s Inventory[{id:"minecraft:gunpowder"}].components."minecraft:damage"

# Clear crafting grid using components
...
# Reinsert item with PROVEN component syntax
execute as @a[tag=crafting_player] if score @s temp_damage matches ..4 run give @s minecraft:gunpowder[minecraft:custom_data={custom_model_data:1004},minecraft:damage=1] 1
